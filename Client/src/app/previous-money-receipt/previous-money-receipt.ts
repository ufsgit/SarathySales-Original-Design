...
export class PreviousMoneyReceipt implements OnInit, OnDestroy {
  ...
  loadData(): void {
    const user = this.api.getCurrentUser();
    if (!user) {
      this.errorMsg.set('User information missing. Please re-login.');
      return;
    }
    ...
    this.api.listMoneyReceipts(this.page(), this.limit(), this.searchTerm(), branchId).subscribe({
      next: (res) => {
