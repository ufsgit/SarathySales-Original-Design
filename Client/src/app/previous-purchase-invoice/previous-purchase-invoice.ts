...
@Component({
  selector: 'app-previous-purchase-invoice',
  standalone: true,
  imports: [UserNav, UserFooter, RouterLink, FormsModule],
  templateUrl: './previous-purchase-invoice.html',
  styleUrl: './previous-purchase-invoice.css',
})
export class PreviousPurchaseInvoice implements OnInit, OnDestroy {
  ...
  loadData(): void {
    ...
    this.errorMsg.set('');
    this.api.listPurchaseInvoices(this.page(), this.limit(), this.searchTerm(), user.branch_id).subscribe({
      next: (res) => {
