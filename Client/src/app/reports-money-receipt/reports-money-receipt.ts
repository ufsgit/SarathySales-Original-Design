...
@Component({
    selector: 'app-reports-money-receipt',
    standalone: true,
    imports: [CommonModule, FormsModule, UserNav, UserFooter, RouterLink],
    templateUrl: './reports-money-receipt.html',
    styleUrl: './reports-money-receipt.css',
})
export class ReportsMoneyReceipt implements OnInit {
    ...
    loadData() {
        ...
        this.api.getMoneyReceiptReport(
            this.branchId(),
            this.fromDate(),
            this.toDate(),
            this.page(),
            ...
            error: (err) => {
                console.error('Error loading money receipt report:', err);
                this.loading.set(false);
            }
        });
    }
    ...
    exportToExcel() {
        ...
        const url = this.api.getMoneyReceiptExcelUrl(branchId, from, to);
    ...
    exportPagedExcel() {
        ...
        const url = this.api.getMoneyReceiptPagedExcelUrl(branchId, from, to, page, limit);
        window.open(url, '_blank');
    }
    ...
    exportPagedCsv() {
        ...
        const page = this.page();
        const limit = this.limit();
        const url = this.api.getMoneyReceiptPagedCsvUrl(branchId, from, to, page, limit);
        window.open(url, '_blank');
    }
}
