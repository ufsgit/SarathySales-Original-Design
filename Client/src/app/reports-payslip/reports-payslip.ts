...
@Component({
    selector: 'app-reports-payslip',
    standalone: true,
    imports: [CommonModule, FormsModule, UserNav, UserFooter, RouterLink],
    templateUrl: './reports-payslip.html',
    styleUrls: ['./reports-payslip.css']
})
export class ReportsPayslip implements OnInit {
    ...
    loadData() {
        ...
        this.api.getPayslipReport(
            ...
            error: (err) => {
                console.error('Error loading payslip report:', err);
                this.loading.set(false);
            }
        });
    }
    ...
    exportToExcel() {
        ...
        const url = this.api.getPayslipExcelUrl(branchId, from, to);
        window.open(url, '_blank');
    }
    ...
    exportPagedExcel() {
        ...
        const url = this.api.getPayslipPagedExcelUrl(branchId, from, to, page, limit);
        window.open(url, '_blank');
    }
    ...
    exportPagedCsv() {
        ...
        const page = this.page();
        const limit = this.limit();
        const url = this.api.getPayslipPagedCsvUrl(branchId, from, to, page, limit);
        window.open(url, '_blank');
    }
}
